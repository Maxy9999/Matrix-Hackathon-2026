...
YOLO_IMGSZ = 480                    # Reduced from 640 to 480 for a massive speedup on RTX 2050

# Detection & ReID params optimized for RTX 2050 (4GB VRAM + Tensor Cores)
YOLO_MODEL_NAME = "yolo11m.pt"
DEEPSORT_EMBEDDER = "clip_ViT-B/16" 
CONF_THRES = 0.55                   
MIN_BBOX_AREA = 2000                
GLOBAL_DIST_THRESH = 0.18           
SHOW_WINDOW = True
...
def main():
    ...
    logger.info(f"Loading YOLO Model: {YOLO_MODEL_NAME} at imgsz={YOLO_IMGSZ}")
    ...
    try:
        while True:
            ...
            any_ok = True
            batch_detections = {cid: [] for cid in valid_cams}
            per_cam_results = {}

            # Run YOLO on every frame in a batch
            results = model(batch_frames, conf=CONF_THRES, imgsz=YOLO_IMGSZ, verbose=False, device=device, half=(device != "cpu"))
            ...
            for idx, res in enumerate(results):
                ...
                for box in res.boxes:
                    ...
                    if box_w * box_h < MIN_BBOX_AREA: 
                        continue
                        
                    dets.append(([x1, y1, box_w, box_h], conf, cls_id))
                
                batch_detections[cid] = dets
            ...
            for cam_cfg in CAMERAS:
                ...
                h, w = frame.shape[:2]
                detections = batch_detections[cam_id]

                # Target lock logic: Make robust to PTZ movements
                if selected_gid is not None and selected_cam_id == cam_id and cam_id in locked_target_bbox:
                    ...
                    ptz_client = ptz_clients.get(cam_id)
                    ptz_moving = ptz_client is not None and ptz_client.last_move_active

                    for det_bbox, det_conf, det_cls_id in detections:
                        det_l, det_t, det_w, det_h = det_bbox
                        det_cx = det_l + det_w / 2.0
                        det_cy = det_t + det_h / 2.0
                        det_area = max(1.0, det_w * det_h)

                        det_area_ratio = min(det_area, last_area) / max(det_area, last_area)

                        if ptz_moving:
                            if det_area_ratio < 0.2:
                                continue
                            
                            dist_to_center = float(np.sqrt((det_cx - w/2.0)**2 + (det_cy - h/2.0)**2))
                            score = (dist_to_center / max(w, h)) + 0.5 * (1.0 - det_area_ratio) - 0.1 * det_conf
                        else:
                            dist_to_last = float(np.sqrt((det_cx - last_cx) ** 2 + (det_cy - last_cy) ** 2))
                            max_dist = max(350.0, float(np.sqrt(last_area) * 3.0))
                            ...
                            if dist_to_last > max_dist and iou < 0.05:
                                continue
                            if det_area_ratio < 0.25:
                                continue

                            score = (dist_to_last / max_dist) - 0.35 * iou + 0.15 * (1.0 - det_area_ratio) - 0.05 * det_conf
            ...
            for cam_id, data in per_cam_results.items():
                ...
                for track in tracks:
                    ...
                    # Disable visualization of others ONLY in the camera where the target is currently locked
                    if sel_gid is not None and sel_cam == cam_id and gid != sel_gid:
                        continue 

                    # Auto-recover Track ID if DeepSORT split the track during a fast PTZ move
                    with state_lock:
                        if selected_gid is not None and selected_cam_id == cam_id:
                            if gid == selected_gid and tid != selected_track_id:
                                selected_track_id = tid
                                sel_tid = tid
                                logger.info(f"[RECOVER] Auto-updated selected track ID to {tid} during PTZ move")
            ...
            if fps_counter % 30 == 0:
                ...
                logger.info(f"[PERF] Processing loop FPS: {30/elapsed:.1f}")
                fps_timer = time.time()
